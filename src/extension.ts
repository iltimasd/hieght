...
export function activate(context: vscode.ExtensionContext) {
  ...
  function check_ifIdWithoutNotNull(
    rangesToDecorate: vscode.DecorationOptions[],
  ) {
    let regex = /if ?\(([^=)]*?I[dD](?!\.)\b) ?[^=<>\r\n]*?\)/g
    let hoverMessage =
      'An ID of 0 would evaluate to falsy. !(0) is truthy. Consider: ${match[1]} != null'
    let popupMessage =
      'ID of 0 would evaluate to falsy. Consider adding "!= null" for if-statements containing IDs: ${errors.join(", ")}'
    genericCheck(regex, hoverMessage, popupMessage, rangesToDecorate)
  }

  function check_ifAssignInsteadOfEquals(
    rangesToDecorate: vscode.DecorationOptions[],
  ) {
    let regex = /if ?\((\w+[^=!<>\r\n])*=([^=\r\n]*)\)/g
    let hoverMessage =
      'Should be ${match[1]} == ${match[2]} or ${match[1]} === ${match[2]}'
    let popupMessage =
      '= should be == or === in if-statements: ${errors.join(", ")}'
    genericCheck(regex, hoverMessage, popupMessage, rangesToDecorate)
  }
  ...
  function check_httpGet(rangesToDecorate: vscode.DecorationOptions[]) {
    ...
    let hoverMessage =
      'For IE, use $http.post instead of $http.get, even for GET, and include an empty request object'
    let popupMessage = 'Use $http.post instead of $http.get (for IE)'
    genericCheck(regex, hoverMessage, popupMessage, rangesToDecorate)
  }

  function check_scopeIdentitySQL(
    rangesToDecorate: vscode.DecorationOptions[],
  ) {
    let regex = /SCOPE_IDENTITY\(\)/g
    let hoverMessage =
      "SCOPE_IDENTITY() is not thread-safe in SQL. Try using the output of an INSERT, e.g.: INSERT INTO MyTable (letters, numbers) OUTPUT INSERTED.[ID] VALUES ('ABC', 123)"
    let popupMessage =
      'SCOPE_IDENTITY() is not thread-safe in SQL. Try using OUTPUT INSERTED.[ID] inside an INSERT.'
    genericCheck(regex, hoverMessage, popupMessage, rangesToDecorate)
  }
  ...
  function check_rowCountSQL(rangesToDecorate: vscode.DecorationOptions[]) {
    ...
    let hoverMessage =
      '@@ROWCOUNT is not very thread-safe in SQL. Try this: NOT EXISTS(SELECT 1 FROM ...)'
    let popupMessage =
      'Instead of @@ROWCOUNT, try NOT EXISTS(SELECT 1 FROM ...)'
    genericCheck(regex, hoverMessage, popupMessage, rangesToDecorate)
  }
  ...
  function check_encryptedSQL(rangesToDecorate: vscode.DecorationOptions[]) {
    ...
    let hoverMessage =
      'AS BEGIN should have "AS--WITH ENCRYPTION" and "BEGIN  -- comment: sp_password"'
    let popupMessage =
      'AS BEGIN should have "AS--WITH ENCRYPTION" and "BEGIN  -- comment: sp_password"'
    genericCheck(regex, hoverMessage, popupMessage, rangesToDecorate)
  }
  ...
  function check_numberOfId(rangesToDecorate: vscode.DecorationOptions[]) {
    ...
    let hoverMessage =
      'If ${match[1]} is an ID but is empty, then Number(${match[1]}) would evaluate to 0 (e.g. Number("") -> 0).'
    let popupMessage =
      'Number(someId) evaluates to 0 if someId is empty (e.g. "").'
    genericCheck(regex, hoverMessage, popupMessage, rangesToDecorate)
  }
  ...
  function randomIeReminder() {
    ...
    if (randomNumber === 1) {
      ...
      message =
        'Random reminder from Batman: Test in IE. Your users deserve it.'
    } else if (randomNumber === 4) {
      message =
        'Random reminder from Gandalf: Test in IE. Or it shall not pass.'
    } else if (randomNumber === 5) {
      message =
        "Random reminder from Elsa: Did you test in IE? Don't let it go."
    } else if (randomNumber === 6) {
      message =
        "Random reminder from Ash: Always test in IE. Catch all 'em bugs."
